...
| `apps/api`           | NestJS 11 (ESM, Express 5 adapter), pino, `@nestjs/throttler`, `@nestjs/schedule`, `prom-client`, `@opentelemetry/sdk-node`, Better Auth | 4000 | `/health/live` + `/health/ready`, `/metrics` (Prometheus — default Node metrics + HTTP request histogram + counter, labeled by route template), `/api/auth/*` (Better Auth, memory or Drizzle adapter; per-route rate limit caps sign-in/sign-up at 5 / 15 min / IP), CORS via env-driven `CORS_ORIGINS` allowlist, per-request access log middleware, graceful shutdown drains in-flight requests (`SHUTDOWN_TIMEOUT_MS`), OTel opt-in via `OTEL_EXPORTER_OTLP_ENDPOINT`, global 100 req/min/IP throttle, optional scheduled jobs behind `JOBS_ENABLED=true`. The HTTP surface is documented in [`docs/api/`](./api/) per ADR [0011](./adr/0011-api-docs-in-markdown.md) (no `/openapi.json` or `/docs` UI). `TenantModule`, `PolicyModule`, and `AuditModule` are wired with noop defaults so handlers can `@Inject(TENANT_RESOLVER)` / `@Inject(POLICY_EVALUATOR)` / `@Inject(AUDIT_SINK)` today; activation = swap the provider value (recipes: enable-multi-tenancy, enable-policy, enable-audit). Reference `notes` module shows the contract → controller → service → test loop; `me` module shows the protected-route pattern. |
