...
| `apps/api`           | NestJS 11 (ESM, Express 5 adapter), pino, `@nestjs/throttler`, `prom-client`, `@opentelemetry/sdk-node`, Better Auth | 4000 | `/health/live` + `/health/ready`, `/metrics` (Prometheus), `/api/auth/*` (Better Auth, memory adapter — swap to Drizzle for prod), OTel opt-in via `OTEL_EXPORTER_OTLP_ENDPOINT`, global 100 req/min/IP throttle. Reference `notes` module under `apps/api/src/notes/` shows the contract → controller → service → test loop. |
