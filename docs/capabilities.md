...
| `apps/api`           | NestJS 11 (ESM, Express 5 adapter), pino, `@nestjs/throttler`, `prom-client`, `@opentelemetry/sdk-node` | 4000 | `/health/live` + `/health/ready`, `/metrics` (Prometheus), OTel opt-in via `OTEL_EXPORTER_OTLP_ENDPOINT`, global 100 req/min/IP throttle. |
...
| `@repo/contracts`      | API envelope, error schema, ids, pagination, domain event shape.                                                      |
