...
| App                  | Stack                                                                                                                                    | Local dev URL                                                   | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/web`           | Next.js 16 (App Router), React 19, Tailwind 4, TanStack Query, Zustand                                                                   | `https://web.fullstack-typescript-template.localhost`           | Standalone build output for slim Docker images. `pnpm --filter @repo/web analyze` runs `@next/bundle-analyzer` behind `ANALYZE=true`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `apps/api`           | NestJS 11 (ESM, Express 5 adapter), pino, `@nestjs/throttler`, `@nestjs/schedule`, `prom-client`, `@opentelemetry/sdk-node`, Better Auth | `https://api.fullstack-typescript-template.localhost`           | `/health/live` + `/health/ready`, `/metrics` (Prometheus — default Node metrics + HTTP request histogram + counter, labeled by route template), `/api/auth/*` (Better Auth, memory or Drizzle adapter; per-route rate limit caps sign-in/sign-up at 5 / 15 min / IP), CORS via env-driven `CORS_ORIGINS` allowlist, per-request access log middleware, graceful shutdown drains in-flight requests (`SHUTDOWN_TIMEOUT_MS`), OTel opt-in via `OTEL_EXPORTER_OTLP_ENDPOINT`, global 100 req/min/IP throttle, optional scheduled jobs behind `JOBS_ENABLED=true`. The HTTP surface is documented in [`docs/api/`](./api/) per ADR [0011](./adr/0011-api-docs-in-markdown.md) (no `/openapi.json` or `/docs` UI). `TenantModule`, `PolicyModule`, `AuditModule`, `OutboxModule`, `QueueModule`, `NotifierModule`, and `StorageModule` are all wired with noop defaults so handlers can `@Inject` the corresponding tokens (`TENANT_RESOLVER` / `POLICY_EVALUATOR` / `AUDIT_SINK` / `OUTBOX_RELAY` / `JOB_QUEUE` / `NOTIFIER` / `OBJECT_STORAGE`) today; activation = swap the provider value (recipes: enable-multi-tenancy, enable-policy, enable-audit, enable-outbox-relay, enable-job-queue, enable-notifier, enable-object-storage). Reference `notes` module shows the contract → controller → service → test loop; `me` module shows the protected-route pattern. |
| `apps/desktop`       | Vite + React 19, Tauri 2 (config only)                                                                                                   | `https://desktop.fullstack-typescript-template.localhost`       | `pnpm dev:desktop` boots the browser shell. The Tauri Rust shell is **not pre-scaffolded** — `apps/desktop/src-tauri/` ships only `tauri.conf.json`. Run `pnpm tauri init` (or `cargo tauri init`) inside `apps/desktop` to generate `Cargo.toml` + `src/main.rs` before `pnpm tauri build`. Picking the bundle identifier, IPC surface, and code-signing identity is fork territory.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `apps/mobile`        | Expo SDK 55 + React Native 0.83, Expo Router                                                                                             | `http://localhost:8081`                                         | iOS, Android, and web export from one source. `apps/mobile/eas.json` ships build profiles with credential placeholders (`appleId`, `ascAppId`, `appleTeamId`, `serviceAccountKeyPath` are `null`); fill them with your Apple/Google credentials before running `eas build`. Auth + API client wiring are scaffold-only and a fork-time decision.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `apps/mfe-host`      | Vite + React, manifest-driven runtime composition                                                                                        | `https://mfe.fullstack-typescript-template.localhost`           | Loads remotes via fetch + dynamic import + custom element.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `apps/mfe-dashboard` | Vite library mode, shadow DOM custom element                                                                                             | `https://mfe-dashboard.fullstack-typescript-template.localhost` | Canonical MFE remote example.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Tenancy          | Global tenant middleware in `apps/api/src/main.ts` (per ADR 0013). Default resolver is `noopTenantResolver`; activate via [enable-multi-tenancy recipe](./recipes/enable-multi-tenancy.md). |
...
- **OpenAPI / Swagger / Scalar UI.** API documentation lives in [`docs/api/`](./api/) as
  hand-authored Markdown — see ADR
  [0011 — API documentation in Markdown](./adr/0011-api-docs-in-markdown.md). Forks targeting a
  public consumer-facing API or needing client codegen (Swagger Codegen, OpenAPI TypeScript) should
  add `zod-openapi` (Zod 4 native) at build time and re-introduce a generator.
