...
function MfeHostApp() {
  ...
  useEffect(() => {
    ...
    async function loadRemote() {
      try {
        const response = await fetch(mfeHostEnv.VITE_MFE_DASHBOARD_MANIFEST_URL, {
        ...
        await importRemoteEntry(
          resolveRemoteEntryUrl(mfeHostEnv.VITE_MFE_DASHBOARD_MANIFEST_URL, manifest),
        );
    ...
    function onReady(event: Event) {
      if (event instanceof CustomEvent) {
        window.dispatchEvent(
          new CustomEvent("repo:mfe-host:remote-ready", { detail: event.detail }),
        );
      }
    }

    window.addEventListener(microFrontendEventNames.ready, onReady);
    return () => window.removeEventListener(microFrontendEventNames.ready, onReady);
  }, []);
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-md border border-slate-200 bg-white p-5">
          ...
          <p className="mt-2 text-slate-600 text-sm">
            The host reads a manifest URL from env and loads the remote entry at runtime.
          </p>
        </div>
        <div className="rounded-md border border-slate-200 bg-white p-5">
...
function RemoteSlot({ remote }: { remote: RemoteState }) {
  ...
  if (remote.status === "error") {
    return <EmptyState description={remote.message} title="Remote failed to load" />;
  }
